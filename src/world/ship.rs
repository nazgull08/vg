use bevy::prelude::*;
use bevy_rapier3d::{prelude::{Collider, RigidBody, Restitution, CoefficientCombineRule}, render::ColliderDebugColor};

use crate::control::types::Selected;

use super::{Game, Cell, Vec3i32};
...
pub fn ship_startup(mut commands: Commands,
...
// this material renders the texture normally
    ...
    game.ground = Some(commands.spawn(PbrBundle {
    ...
    game.map.cells = (0..SHIP_X)
        .map(|i| {
            (0..SHIP_Y)
                .map(|j| {(0..SHIP_Z).map(|k|{
                    let mut rng = rand::thread_rng();
                    let rwall: f64 = rng.gen();
                    if (k < 1) || ((k < 2) && (rwall >0.5)) || ((k < 3) && (rwall >0.7)) || ((k >= 3) && (rwall >0.9)) {

                    info!("k: {k}, rw: {rwall}");
                   game.players.push(Some( commands.spawn(PbrBundle {
                        mesh: meshes.add(Mesh::from(shape::Box{max_x:1.
                        ,min_x:-1.
                        ,min_y:-1.
                        ,max_y:1.
                        ,min_z:-1.
                    ...
                    Cell{position : Vec3i32::new(i,j,k), object: None}
                }).collect()
                })
                .collect()
        })
